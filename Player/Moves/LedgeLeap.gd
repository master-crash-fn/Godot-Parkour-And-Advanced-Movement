...
var needs_final_correction : bool = true
...
func move_player(delta):
	if get_progress() > 0.35 and get_progress() < 1.13:
		var target_point_2d : Vector2 = calculate_trajectory((get_progress() - 0.35) / (1.13-0.35))
		var target_point_3d : Vector3 = (end_position - start_position)
		target_point_3d.y = 0
		target_point_3d = target_point_3d.normalized() * target_point_2d.x
		target_point_3d.y =  target_point_2d.y
		...
		player.basis.z = start_basis_dir.slerp(grab_direction, (get_progress() - 0.35) / (1.13-0.35)).normalized()
		player.basis.x = Vector3.UP.cross(player.basis.z)
		return
	if get_progress() >= 1.13 and needs_final_correction:
		print("final")
		needs_final_correction = false
...
func on_enter_state(input : InputPackage):
	...
	leap_direction.y = 0
	grab_direction = Vector3.UP.cross(area_awareness.current_ledge[0].direction_to(area_awareness.current_ledge[1])).normalized()
	...
	area_awareness.last_wall_jump_normal = grab_direction
